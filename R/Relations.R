...
makeCEPH <- function(id, sire, dam){
  ...
  # Return
  # ------
  # list of lists {fields: id, {subfields: parents, pgp, mgp}}
  ...
  ped <- data.frame(sire=sire, dam=dam, row.names=id, stringsAsFactors=FALSE)
...
convertRelationships <- function(kmat, ped, ids=NULL, updateProgress=NULL){
  ...
  for(i in 1:nrow(kin)){
    ...
    } else if(id1 %in% c(ceph2$pgp, ceph2$mgp) | id2 %in% c(ceph1$pgp, ceph1$mgp)){
      # one animals is the grandparent of the other
      relation <- "Grandparent-Grandchild"
    } else if(entire(ceph1$pgp == ceph2$pgp) | entire(ceph1$pgp == ceph2$mgp) |
              entire(ceph1$mgp == ceph2$pgp) | entire(ceph1$mgp == ceph2$mgp)){
