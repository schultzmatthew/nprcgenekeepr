...
test_that("getProductionStatus calculates correctly", {
  status <- getProductionStatus(
    ped, minParentAge = 3, maxOffspringAge = NULL, housing = "shelter_pens",
...
test_that("getProductionStatus calculates correctly", {
  status <- getProductionStatus(
    ped, minParentAge = 3, maxOffspringAge = NULL, housing = "corral",
    currentDate = as.Date("2010-10-10", format = "%Y-%m-%d"))
  expect_equal(status$production, 0.112877583465819)
  expect_equal(status$color, "red")
})
## These are the IDs from the test above
ids <- c("Q3J24E", "IC716S", "SJD499", "YJBY17", "5C63F4", "R3R07C",
...
test_that("getProductionStatus calculates correctly", {
  status <- getProductionStatus(
    pedWith71, minParentAge = 3, maxOffspringAge = NULL,
...
test_that("getProductionStatus calculates correctly", {
  status <- getProductionStatus(
    pedWith71OffspringWithDams, minParentAge = 3, maxOffspringAge = NULL,
    housing = "shelter_pens",
    currentDate = as.Date("2010-10-10", format = "%Y-%m-%d"))
...
test_that("getProductionStatus calculates correctly", {
  status <- getProductionStatus(
    yellowCorralPed, minParentAge = 3, maxOffspringAge = NULL,
    housing = "corral",
    currentDate = as.Date("2010-10-10", format = "%Y-%m-%d"))
...
yellowShelterPedPed <- pedWith71OffspringWithDams[
  !pedWith71OffspringWithDams$id %in% ids[1:33], ]
test_that("getProductionStatus calculates correctly", {
  status <- getProductionStatus(
    yellowShelterPedPed, minParentAge = 3, maxOffspringAge = NULL,
    housing = "shelter_pens",
    currentDate = as.Date("2010-10-10", format = "%Y-%m-%d"))
