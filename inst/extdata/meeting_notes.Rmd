...
1. Change Update breeding colony button to "" Waiting on Amanda. Changed to "Update Focal Animals" based on instructions from Amanda. 2018-08-04
2. Add function to "Update Breeding Colony" button to browse for a file; add animal IDs to the window where they could be pasted. Started 2018-07-29 getting file not updating window.
...
2. Amanda and Beth will provide me with some files with errors.
   a. Amanda provided a file with two error types: bad birth date type - integer instead of character representation of a date and animal appearing as a sire and a dam. 20180720
   ...
   _Pedigree_File_Example_134M_dam_removed.csv_, and _Pedigree_File_Example_CSV.csv_ 20180721
...
Also added unit tests for __convertDate__. 20180721
5. Added ability to handle dates with NA to __convertDates__ 20180722
6. Added unit test coverage of __updateProgress__ calls in _test_reportGV.R_ 20180722
7. Corrected a bug and added corralary unit test for __rankSubjects__.
8. Added test for missing __age__ column in _orderReport_ function. This also allows correct
ordering when the age column is missing. 2018-07-29
9. Added unit tests for _findLoops_ function.
10. Added check for class on objects in _convertDates_ function, because it was failing when 
objects were already dates. It now passes those objects through untouched. 2018-08-04
11. Added some debug code for updating focal animals (still named breeding colony update in code) 2018-08-15
