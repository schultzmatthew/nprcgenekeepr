...
## 20191130
1.  See if https://www.semanticscholar.org/paper/Management-of-genetic-diversity-using-gene-dropping-Khaldari-Javaremi/43dd1975193830b9243c8eef672db134e42e871d/figure/3 
...
1.  Provide a way to clear focal animals added in __Pedigree Browser__ tab
    so that full pedigree is again availabe. This will also allow a new 
...
originalPath <- stri_c("/Users/msharp/Documents/Projects/Active_Projects/",
                       "nprcmanager_project/20160816_GeneticManagementTools")
functions <- stri_c("/Users/msharp/Documents/Development/R/r_workspace/",
                    "library/nprcmanager/R")
tests <- stri_c("/Users/msharp/Documents/Development/R/r_workspace/library/",
                "nprcmanager/tests/testthat")
ui <- stri_c("/Users/msharp/Documents/Development/R/r_workspace/library/",
             "nprcmanager/inst/application")

paths <- c(originalPath, functions, ui, tests)
...
codeChanges <- data.frame(Source = c("Original", "nprcmanager 0.37"),
                          Files = c(codeCounts$files[1],
                                    sum(codeCounts$files[2:4])),
                          Lines = c(codeCounts$lines[1],
                                    sum(codeCounts$lines[2:4])),
                          Code = c(codeCounts$code[1],
                                    sum(codeCounts$code[2:4])),
                          `Blank Lines` = c(codeCounts$blank_lines[1],
                                    sum(codeCounts$blank_lines[2:4])),
                          `Document Lines` = c(codeCounts$roxygen_doc_lines[1],
                                    sum(codeCounts$roxygen_doc_lines[2:4])),
                          `Comment Lines` = c(codeCounts$comments[1],
                                    sum(codeCounts$comments[2:4])),
                          check.names = FALSE,
                          stringsAsFactors = FALSE)

```
```{r code-change-table, echo = FALSE}
kable(codeChanges, caption = stri_c("Simple metrics for the Original Version (",
                                    "20160816) and Version ", getVersion(),
                                    "."))
...
2.  We discussed heat map type issues with regard to realtime examination of 
    kinship relationships within breedeing groups
...
1. Work on loop code: have list of animals involved in loops with counts, number of loops, number of animals in loops.
   Not high priority.
1. DCM wants to report on problems in current breeding groups prior occurance.
...
1. List of things found prior to the meeting to do before next meeting:
 ...
 We have interpretted this as meaning that genome uniqueness should only be 
...
1. Items to add to instructions:
   a.  Discuss what to do about overlapping group formation. 
     ...
     but will be mutually exclussive within a run.
...
1. Discussed what to do about overlapping group formation. 
  ...
  a new set of groups. The new set will overlap with the prior results, 
  but will be mutually exclussive within a run. Added to things to do
...
1. Amanda and Mark had fairly extensive discussions regarding how group
  ...
  potential overlay among the various subgroups. This is complecated
...
10. Miscellaneous
  ...
  - Worked with Terry Hawkins on 20181119 and 20181120 to make sure the 
    ...
    message was misleading. I have trapped the erorr with a tryCatch 
