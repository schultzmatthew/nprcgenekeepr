...
#	descendant population contributed by founder <MATH>i</MATH>."))
  ...
  bg <- eventReactive(input$grpSim, {
    ...
    if (groupNum != "0") {
      ...
      currentGroup[[groupNum]] <- unlist(strsplit(input$curGrp, "[ \t\n]"))
      if (length(ids) > 0) {
      ...
      if (length(currentGroup[[groupNum]]) > 0) {
        candidates <- setdiff(candidates, currentGroup[[groupNum]])
      }
      ...
      validate(
        ...
        need(!(length(setdiff(currentGroup[[groupNum]], ped$id)) > 0),
             paste("Current group members present that",
                   "are not in the provided pedigree\n",
                   paste(setdiff(currentGroup[[groupNum]], ped$id), sep = "\n")))
      )
      ...
      if (length(currentGroup[[groupNum]]) > 0) {
        grp <- groupAddAssign(candidates = candidates,
                              currentGroup = currentGroup[[groupNum]],
                              kmat = kmat(),
                              ped = ped,
                              threshold = threshold,
                              ignore = ignore,
