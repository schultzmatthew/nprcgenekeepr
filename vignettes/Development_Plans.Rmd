...
```
## Next steps
1.  Allow the user to provide multiple seed groups for the breeding groups.
    1.  Have user select number of breeding groups to form.
    1.  Have user place seed animals in any number of those groups.
    1.  Calculate the average number of animals in each group and round down
        to the nearest integer (truncate). Use this integer for the total number
        of animals to place in the groups with seed animals.
    1.  Loop through each of the groups with seed animals placing the single
        best fitting animal from the candidate list in that group 
        (young pre-weanlings animals are 
        being ignored with regard to selection and count) until all of 
        these groups are filled. 
    1.  Form the best possible groups with the remaining animals by selecting
        the groups which provide the lowest mean kinship coefficients. This
...
1. There are some functions that are defined and not used. 
...
1. Consider developing a simulation module to allow uses to simulate breeding decisions.
...
1. It would be better to catalogue and report the changes made by qcStudbook()
   ...
   developing the functions __summary.nprcgenekeeprErr__ and
   __print.summary.nprcgenekeeprErr__, which when combind with using 
