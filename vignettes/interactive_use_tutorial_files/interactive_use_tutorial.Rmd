...
- __byID__ Logical varioable of length 1 that is passed through to eventually 
...
The primary purpose of __nprcmanager__ is to form breeding groups 
...
__nprcmanager__, which we will examine using code below.

-   Animals used to form groups
    -   _high-value_: Randomly select from only high-value animals in genetic value analysis
...
assigned to groups. Defaults to a list with character(0) in each sublist 
...
0.5 to 10 by increments of 0.5.
...
kinship matrix. This maintains compatability with earlier versions.
...
```{r get-candidates}
candidates <- read.table(file = system.file("testdata", 
                                            "trim_Example_Pedigree_IDs.txt",
                                            package = "nprcmanager"))
...
```
Our candidates are made up of `r table(trimmedPed$sex[trimmedPed$id %in% candidates])[[1]]`
females and `r table(trimmedPed$sex[trimmedPed$id %in% candidates])[[2]]` males.


```{r identify-candidate-animals, eval = FALSE}
...
First note the birthdates of ego_id _o4_ 
(2006-04-13) and the purported sire _s2_ (2006-06-19). Note also the proported 
birthdate of the _d2_ and the birthdates of her offspring.
Obviously dates or IDs may be incorrect.

This is the pedigree. _(We will discuss the column names shortly.)_
```{r list-pedOne}
knitr::kable(nprcmanager::pedOne)
```
...
creation of a file in the user's home directory that lists the records that 
have generated the errors. 
...
lowParentAge <- read.csv("~/lowParentAge.csv")
```
...
We can remove the errors by changing the birthdates of _o4_ from 2006-04-13
...
before proceeding.
This is done by explicitely requesting that all aspects inconsistent with the 
