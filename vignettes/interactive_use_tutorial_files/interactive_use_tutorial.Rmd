...
If we examine this pedigree file, we are going to get an error message. 

```{r summary-pedOne-no-errors, error = TRUE}
pedOne <- nprcmanager::pedOne # put it in the local environment
qcStudbook(pedOne, minParentAge = 0)[pedOne$ego_id %in% c("s2", "d2", "o3",
                                                          "o4"), ]

```

We can remove the error by correcting the birthdate of _o4_ from 2006-04-13
to 2015-09-16 and of _d2_ from 2015-09-16 to 2006-04-13. 

Note that we can display some of the rows by selecting the _ego_id_'s to
include.
