...
```{r example-pedigree-min-parent-age, eval = TRUE, fig.width = 2.5, fig.height = 3.5, echo = FALSE}
img <- readPNG("../../inst/extdata/shiny_app_tutorial/images/input_example_pedigree_min_parent_age.png")
grid.raster(img)
```
...
```{r example-pedigree-min-parent-age-4, eval = TRUE, fig.width = 2.5, echo = FALSE}
img <- readPNG("../../inst/extdata/shiny_app_tutorial/images/input_minParentAge_4.png")
grid.raster(img)
```
...
```{r example-pedigree-min-parent-age-2, eval = TRUE, fig.width = 2.5, fig.height = 3.5, echo = FALSE}
img <- readPNG("../../inst/extdata/shiny_app_tutorial/images/input_minParentAge_2.png")
grid.raster(img)
```
...
You can choose to display UNKNOWN IDs in the rows diplayed.
...
You can also import a list of focal animals by selecting the __Browse__ button
...
animal IDs or animal IDs on separate lines.
...
This is showing living animals from a larger pedigree than the one listed 
immediately above.
...
You can select how many rows to display at once by changing the values in the 
